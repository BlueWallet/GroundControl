...
export class GroundControlToMajorTom {
    ...
    protected static async _pushToApns(apnsPem: string, token: string, apnsPayload: object, pushNotification: Components.Schemas.PushNotification) : Promise <[object, object]> {
        return new Promise(function(resolve) {
            ...
            request.on('end', () => {
                ...
                PushLogRepository.save( { token: token, payload: JSON.stringify(apnsPayload), response: JSON.stringify(responseJson), success: responseJson[':status'] === 200 });
    ...
    protected static async _pushToFcm(serverKey: string, token: string, fcmPayload: object, pushNotification: Components.Schemas.PushNotification) : Promise <[object, object]> {
        ...
        const PushLogRepository = getRepository(PushLog);
        await PushLogRepository.save( { token: token, payload: JSON.stringify(fcmPayload), response: JSON.stringify(responseJson), success: !!responseJson['success'] });
